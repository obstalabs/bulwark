...
pub struct AllowList {
    ...
    /// file aliased into a grant path after launch — hardlinked (B2) or renamed
    /// (A-2) in from outside — presents a foreign inode and is denied. Empty
...
impl AllowList {
    ...
    fn snapshot_one(&mut self, g: &str, path: &Path, depth: usize) {
        ...
        if glob::matches(g, &path_str) {
            self.grant_inodes.insert(InodeKey::of(&meta));
        }
        ...
        for entry in entries.flatten() {
            ...
            if is_real_dir {
                ...
                if glob::matches(g, &ep.to_string_lossy()) {
                    self.grant_inodes.insert(InodeKey::of(&m));
                }
            }
        }
    }
    ...
    ///     * hardlink-into-grant (B2): the secret's inode was never under the
    ///       grant at launch, so it is not in the snapshot → denied;
    ///     * rename-into-grant (A-2): same — the renamed inode is foreign.
    ///
    ...
    pub fn allows_open(&self, path: &str, key: &InodeKey) -> bool {
        if self.include_base && RUNTIME_BASE_SET.iter().any(|g| glob::matches(g, path)) {
            return true;
        }
        if self.grants.iter().any(|g| glob::matches(g, path)) {
            return self.grant_inodes.contains(key);
        }
        false
    }
    ...
    #[cfg(test)]
    pub fn insert_grant_inode_for_test(&mut self, key: InodeKey) {
        self.grant_inodes.insert(key);
    }
}
...
mod inode_tests {
    ...
    fn snapshotted_grant_file_is_allowed() {
        ...
        a.snapshot_grants();
        assert!(a.allows_open(&f.to_string_lossy(), &key_of(&f)));
    }

    /// B2/A-2 negative control: a path that matches a grant glob but whose inode
    ...
    fn foreign_inode_on_granted_path_is_denied() {
        ...
        assert!(
            !a.allows_open(&aliased_path, &key_of(&secret)),
    ...
    fn non_matching_sibling_is_not_snapshotted() {
        ...
        assert!(a.allows_open(&log.to_string_lossy(), &key_of(&log)));
        ...
        assert!(!a.allows_open(&aliased, &key_of(&env)));
    }
    ...
    fn base_set_path_allowed_regardless_of_inode() {
        ...
        #[cfg(target_os = "linux")]
        assert!(a.allows_open("/etc/passwd", &any));
        #[cfg(target_os = "macos")]
        assert!(a.allows_open("/usr/lib/dyld", &any));
    }
    ...
    fn ungranted_path_denied() {
        ...
        a.insert_grant_inode_for_test(k);
        assert!(!a.allows_open("/etc/shadow", &k));
    }
}
