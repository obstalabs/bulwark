...
| You see | Cause | Fix |
|---|---|---|
| `sudo: bulwark: command not found` (right after `brew install`) | `sudo` uses a locked-down `secure_path` that does **not** include Homebrew's bin (`/opt/homebrew/bin` on Apple Silicon, `/usr/local/bin` on Intel), so it can't find the CLI. | Run by full path: `sudo "$(which bulwark)" doctor`. For `run` with a Homebrew-installed agent (e.g. a Node-based CLI), also carry your PATH so the agent's runtime is found: `sudo env "PATH=$PATH" "$(which bulwark)" run --protect <path> -- <agent>`. (Plain `sudo -E` does **not** fix the command lookup — it preserves env vars but still resolves the command via `secure_path`.) |
